...
module Echoes
  class Terminal
    ...
    def initialize(command: Echoes.config.shell, rows: nil, cols: nil)
      ...
      # See gui.rb: pin $ECHOES_RUBY so wrappers like rubish's launcher
      # use this interpreter directly and skip the rbenv shim that
      # would otherwise re-export RBENV_VERSION into the child shell.
      require 'rbconfig'
