...
require "test_helper"
require "echoes"
require "echoes/editor"
require "tmpdir"

class Echoes::EditorTest < Test::Unit::TestCase
  def setup
    @dir = Dir.mktmpdir
  ...
  test "loads a file and exposes its lines as styled segments" do
    v = Echoes::Editor.new(file: @ruby_file, rows: 8, cols: 80)
  ...
  test "feed_key 'G' jumps the cursor to the last line" do
    v = Echoes::Editor.new(file: @plain_file, rows: 5, cols: 80)
  ...
  test "feed_key 'j' moves the cursor down one line" do
    v = Echoes::Editor.new(file: @plain_file, rows: 10, cols: 80)
    v.feed_key('j')
  ...
  test "missing file is handled gracefully" do
    v = Echoes::Editor.new(file: '/nonexistent/path.txt', rows: 5, cols: 80)
    rows = v.visible_segments
  ...
  test "visible_segments pads with blank rows up to viewport height" do
    v = Echoes::Editor.new(file: @ruby_file, rows: 20, cols: 80)
    rows = v.visible_segments
    assert_equal 20, rows.size
  end

  # --- Editing path: rvim machinery already does the work; the
  # Editor is the rendering shim. These tests pin down the bits
  ...
  test "i + char + Esc enters insert mode and modifies the buffer" do
    v = Echoes::Editor.new(file: @plain_file, rows: 5, cols: 40)
    v.feed_key('i')
  ...
  test "padding rows render vim-style ~ markers when the buffer is short" do
    ...
    v = Echoes::Editor.new(file: short, rows: 5, cols: 40)
    rows = v.visible_segments
...
class Echoes::PaneEditorTest < Test::Unit::TestCase
  def setup
    require "echoes/pane"
  ...
  test "editor pane renders the file's contents into the screen grid" do
    pane = Echoes::Pane.new(command: '', rows: 8, cols: 40, editor_file: @file)
  ...
  test "editor pane responds to vim navigation keys" do
    pane = Echoes::Pane.new(command: '', rows: 8, cols: 40, editor_file: @file)
  ...
  test "editor pane reports editor? and not embedded?" do
    pane = Echoes::Pane.new(command: '', rows: 8, cols: 40, editor_file: @file)
    assert pane.editor?
    refute pane.embedded?
  end

  test "editor pane is alive and read_available_output is empty" do
    pane = Echoes::Pane.new(command: '', rows: 8, cols: 40, editor_file: @file)
    assert pane.alive?
    assert_equal '', pane.read_available_output
  end

  test "Pane#alive? becomes false after the editor quits via :q" do
    pane = Echoes::Pane.new(command: '', rows: 8, cols: 40, editor_file: @file)
    assert pane.alive?
